...
# The API URL now points to the service running in the background on the same machine
API_URL = "http://127.0.0.1:8000"

# --- Run the FastAPI backend in a separate thread ---
# This ensures the backend starts and runs alongside the Streamlit UI
...
# ... (rest of the Streamlit UI code is the same as your frontend.py)
            ...
            with st.status(f"Processing request for {ticker_to_scrape}...", expanded=True) as status:
                try:
                    ...
                    if response.status_code != 200:
                        st.error("Failed to start the backend job.")
